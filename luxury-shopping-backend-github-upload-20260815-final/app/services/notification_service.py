...
PASSWORD_RESET_NOTIFICATION_TYPES = frozenset({
    "password_reset",
...
def _token_language(token_row: Any) -> str:
    extra_data = getattr(token_row, "extra_data", None)
    if not isinstance(extra_data, dict):
        return "ar"
    value = str(extra_data.get("language") or extra_data.get("locale") or "").strip().lower()
    return "en" if value.startswith("en") else "ar"


def _english_notification_content(row: Any, notification: dict[str, Any]) -> tuple[str, str]:
    """Return English push text for the notification types generated by the app."""
    data = notification.get("data") if isinstance(notification.get("data"), dict) else {}
    title = _clean_notification_text(data.get("title_en"), "")
    body = _clean_notification_text(data.get("body_en"), "")
    notification_type = str(getattr(row, "type", None) or "").strip().lower()
    status = str(data.get("order_status") or data.get("status") or "").strip().lower()
    status_label = _ENGLISH_ORDER_STATUS_LABELS.get(status, status.replace("_", " ").title())
    if notification_type in {
        ...
        title = title or "Order status updated"
        if data.get("finalCost") is not None or data.get("final_cost") is not None:
            total = data.get("finalCost") if data.get("finalCost") is not None else data.get("final_cost")
            currency = data.get("currencyCode") or data.get("currency_code") or "YER"
            body = body or f"Your international order pricing was updated. New total: {total} {currency}."
        else:
            body = body or f"Your order status is now {status_label}."
    elif notification_type == "product_approved":
        title = title or "Your product was approved"
        body = body or "Your product was approved and is now visible to customers."
    elif notification_type == "product_rejected":
        title = title or "Your product was rejected"
        reason = data.get("rejectionReason") or data.get("rejection_reason")
        body = body or (f"Reason for rejection: {reason}" if reason else "Your product was rejected. Please review its details.")
    elif notification_type == "partner_request_approved":
        title = title or "Your special request was approved"
        body = body or "Your request was approved by the administration."
    elif notification_type == "partner_request_rejected":
        title = title or "Your special request was rejected"
        reason = data.get("review_reason") or data.get("reason")
        body = body or (
            f"Reason for rejection: {reason}"
            if reason
            else "Your request was rejected. Please review its details."
        )
    elif notification_type == "product_submitted_for_review":
        title = title or "Your product is under review"
        body = body or "Your product is being reviewed by the administration."
    elif notification_type == "storefront_approved":
        title = title or "Your store was approved"
        body = body or "Your store was approved and is ready for customers."
    elif notification_type == "storefront_rejected":
        title = title or "Your store was rejected"
        reason = data.get("rejectionReason") or data.get("rejection_reason")
        body = body or (f"Reason for rejection: {reason}" if reason else "Your store request was rejected. Please contact support.")
    elif notification_type in {"partner_application_approved", "partner_approved"}:
        title = title or "Your merchant application was approved"
        body = body or "Your merchant account is ready to set up in the app."
    ...
    elif notification_type in {"customer_welcome", "welcome", "welcome_message", "welcome_popup"}:
        title = title or "Welcome to Luxury Shopping"
        body = body or "Welcome. We are happy to have you with us."
    elif notification_type in {"support_reply", "contact_reply", "merchant_reply"}:
        title = title or "New reply from support"
        body = body or "You have a new reply in the app."
    elif notification_type in {"support_ticket", "ticket_opened"}:
        title = title or "Support request received"
        body = body or "Your support request was received and will be reviewed soon."
    elif notification_type in {"offer_published", "promo", "promotion", "marketing", "marketing_campaign", "coupon", "discount"}:
        title = title or "New offer from Luxury Shopping"
        body = body or "A new offer is available in the app."
    elif notification_type in {"email_verification_requested", "email_message"}:
        title = title or "Message from Luxury Shopping"
        body = body or "You have a new message in the app."
    return title, body
...
class NotificationService:
    ...
    async def register_device_token(self, user_id: uuid.UUID, body: dict[str, Any]) -> dict[str, Any]:
        ...
        requested_language = str(body.get("language") or body.get("locale") or "").strip().lower()
        if requested_language:
            row.extra_data = {
                **(row.extra_data if isinstance(row.extra_data, dict) else {}),
                "language": "en" if requested_language.startswith("en") else "ar",
            }
    ...
    async def _allowed_channels(
        ...
        if configured_channels is not None:
            ...
            if str(notification_type or "").strip().lower() in PASSWORD_RESET_NOTIFICATION_TYPES:
                channels = [channel for channel in channels if channel != "mobile_push"]
            return channels
        ...
        # Customer messages are actionable mobile alerts. Keep the heads-up
        # channel enabled even when an old preference row disabled it; password
        # reset remains an email-only security flow by design.
        normalized_type = str(notification_type or "").strip().lower()
        if (
            customer_notification_allowed(normalized_type)
            and normalized_type not in PASSWORD_RESET_NOTIFICATION_TYPES
            and "mobile_push" not in channels
        ):
            channels.append("mobile_push")
    ...
    async def _send_mobile_push(self, row: Any, notification_id: str | None) -> str:
        ...
        for token_row in tokens:
            ...
            if _token_language(token_row) == "en":
                title_en, body_en = _english_notification_content(row, notification)
                if title_en or body_en:
                    localized = {
                        **notification,
                        "title": title_en or notification["title"],
                        "body": body_en or notification["body"],
                        "data": {
                            **notification["data"],
                            "title_en": title_en or notification["title"],
                            "body_en": body_en or notification["body"],
                        },
                    }
            notification_data = {
                key: str(value)
                for key, value in localized["data"].items()
            ...
            notification_data.setdefault("title", localized["title"])
            notification_data.setdefault("body", localized["body"])
            notification_data.setdefault("message", localized["body"])
            ...
            deep_link = str(notification_data.get("deep_link") or localized["url"] or settings.frontend_public_url).strip()
